...
const lightSemantics = JSON.parse(readFileSync(join(tokensDir, 'semantics/light.json'), 'utf8'));
const darkSemantics = JSON.parse(readFileSync(join(tokensDir, 'semantics/dark.json'), 'utf8'));
...
function semanticVars(tokens) {
	return Object.fromEntries(tokens.map(({ cssVar, token }) => [cssVar, resolveSemanticValue(token)]));
}
