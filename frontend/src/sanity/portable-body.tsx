...
import {enrichExperiencesWithSvgMarkup} from '@/sanity/experience-svg-markup'
...
const getExperiencesForExperienceBlock = cache(async () =>
  client.fetch<EXPERIENCE_LIST_QUERY_RESULT>(EXPERIENCE_LIST_QUERY, {}, {next: {revalidate: 30}}),
)
...
export async function PortableBody({body}: {body: PortableBodyValue | null | undefined}) {
  ...
  const [notes, experiencesFetched] = await Promise.all([
  ...
  const experiences =
    experiencesFetched != null ? await enrichExperiencesWithSvgMarkup(experiencesFetched) : null
